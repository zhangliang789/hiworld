...
//        $("#goNewRiskhazardBtn").hide();
        ...
        if ($("#upRiskhazardName").val() == null || $("#upRiskhazardName").val() == '') {
            showToast("error", "风险不得为空！");
            return false;
        }

        if ($("#upRiskType").val() == null || $("#upRiskType").val() == '') {
            showToast("error", "管理类型不得为空！");
            return false;
        }

        if ($("#upMajorType").val() == null || $("#upMajorType").val() == '') {
            showToast("error", "专业类型不得为空！");
            return false;
        }

        var startTime1s=$("#startTime1").val();
        var endTime1s=$("#endTime1").val();
        if (startTime1s >endTime1s) {
            showToast("warning", "管控开始时间不能大于管控结束时间");
            return false;
        }
...
//                alert("新增风险点参数有误！");
    ...
    function provNewRiskHazard() {//验证新建riskSite输入是否正确的方法
        ...
        if ($("#majorType").val() == null || $("#majorType").val() == '') {
            showToast("error", "专业类型不得为空！");
            return false;
        }
