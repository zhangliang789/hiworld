        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="header-section" style="overflow: hidden;width:70%;">
                    <h1 style="float: left;padding:0">风险审核</h1>
            ...
            <div class="">
                <table id="general-table" class="table table-striped table-bordered table-vcenter table-hover">
                    <tr class="biaot">
                        ...
                        <td class="text-center" rowspan="2">审核状态 </td>
                    ...
                    <tr v-for="(risk,riskIndex) in detailedAllList">
                        ...
                        <td class="text-center" v-if="risk.RiskColor!=null&&risk.RiskColor!=''">
                            <span v-bind:style="'background-color:'+risk.RiskColor" class="btys"></span>
                        </td>
                        <td class="text-center" v-if="risk.RiskColor==null||risk.RiskColor==''"></td>
                        <td class="text-center" v-text="risk.RiskType"> </td>
                        <td  v-text="risk.MeasureContent"> </td>
                        <td class="text-center" v-text="risk.MeasureName"> </td>
                        <td class="text-center" v-text="risk.TechnologyDepartment"> </td>
                        <td class="text-center" v-text="risk.SupervisionDepartment"> </td>
                        <td class="text-center" v-text="risk.ControlRangeStartDay"> </td>
                        <td class="text-center" v-text="risk.ControlRangeEndDay"> </td>
                        <td class="text-center" v-text="risk.KJControlRang"> </td>
                        <td class="text-center" v-text="risk.ZYControlRang"> </td>
                        <td class="text-center" v-text="risk.QDControlRang"> </td>
                        <td class="text-center" v-text="risk.BZControlRang"> </td>
                        <td class="text-center" v-text="risk.GRControlRang"> </td>
                        <td class="text-center" v-text="risk.reviewStatus"> </td>
                        <td class="text-center" v-text="">
                            <i-button v-if="risk.reviewStatus=='已审核'" type="primary" disabled>审核</i-button>
                            <i-button v-else type="primary" @click="riskHazardReview(risk.HazardID,risk.mazardID)">审核</i-button>
                        </td>
                    </tr>
                    </tbody>
                </table>
    ...
    var vueApp = new Vue({
        ...
        methods: {
            ...
            getRiskDetailByLiangJia: function () {
                ...
                var endTime = $("#endTime").val();
                $.ajax({
                    url: projectName + "/Templatehazard/getRiskReviewStatusByLiangJia", //发送请求的地址。
                    type: "POST", //(默认: "GET") 请求方式 ("POST" 或 "GET")， 默认为 "GET"。注意：其它 HTTP 请求方法，如 PUT 和 DELETE 也可以使用，但仅部分浏览器支持。
                    data: {
                        "topName": _self.topName,
                        "hazardName":_self.hazardName,
            ...
            riskHazardReview:function(hazardId,mazardId){
                var $this = this;
                $.post(projectName + "/Templatehazard/updateRiskReviewStatusByLiangJia",
                    {"hazardId":hazardId,
                     "measureId":mazardId},
                    function (data) {
                        if(data){
                            $this.showToast("success", "审核成功!");
                            $this.getRiskDetailByLiangJiaCount();
                        }
