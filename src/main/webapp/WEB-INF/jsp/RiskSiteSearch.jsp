            ...
            <div class="block full" style="padding:20px 0 0 0;border: 0;overflow: hidden;">
                ...
                <div class="block-title" style="padding:10px 0;">
                    <ul class="nav nav-tabs" data-toggle="tabs" style="    padding: 0 20px;">
                        ...
                        <li class="">
                            <a @click="getRiskAnalyzeByPlaceType">地点类型</a></li>
                        <div class="" style="float: right;">
                            <a id="dlink"  style="display:none;"></a>
                            <button type="submit" class="btn btn-effect-ripple btn-primary" style="margin: 2px 0px 0px 0px;"
                                    id="genTab" @click="DaoChu('general-table')">导出</button>
                            <button type="submit" class="btn btn-effect-ripple btn-primary" style="margin: 2px 0px 0px 0px;"
                                    id="genTab1" @click="DaoChu('general-table1')" hidden>导出</button>
                        </div>
                    </ul>
                </div>
                        ...
                        <div class="table-responsive">
                            <div id="example-datatable_wrapper1" class="dataTables_wrapper form-inline no-footer">
                                <table id="general-table1"
                                    ...
                                    <thead>
                                    <tr>
                                        <th class="text-center">地点类型</th>
                                        <%--<th class="text-center">风险点名称</th>--%>
                                        <th class="text-center">风险点数量</th>
                                        <th class="text-center">重大风险</th>
                                        <th class="text-center">较大风险</th>
                                        <th class="text-center">一般风险</th>
                                    ...
                                    <tr>
                                        <td class="text-center" v-text="mrisk.riskDamageName"></td>
                                        <%--<td class="text-center"><a--%>
                                        <%--@click="queryRiskSite(mrisk.places,mrisk.hazardCount,'0')"--%>
                                        <%--v-text="mrisk.riskSiteName"></a></td>--%>
                                        <td class="text-center"><a
                                                @click="queryRiskSite(mrisk.places,mrisk.placeCount,'0')"
                                                v-text="mrisk.placeCount"></a></td>
                                        <td class="text-center"><a @click="queryRiskSite(mrisk.places,mrisk.l1,'重大风险')"
                                                                   v-text="mrisk.l1"></a></td>
                                        <td class="text-center"><a @click="queryRiskSite(mrisk.places,mrisk.l2,'较大风险')"
                                                                   v-text="mrisk.l2"></a></td>
                                        <td class="text-center"><a @click="queryRiskSite(mrisk.places,mrisk.l3,'一般风险')"
                                                                   v-text="mrisk.l3"></a></td>
                                        <td class="text-center"><a @click="queryRiskSite(mrisk.places,mrisk.l4,'低风险')"
                                                                   v-text="mrisk.l4"></a></td>
                                        <td class="text-center"><a @click="queryRiskSite(mrisk.places,mrisk.wpg,'未评估')"
                                                                   v-text="mrisk.wpg"></a></td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div>
...
<script>
    ...
    var vueApp = new Vue({
        ...
        methods: {
            ...
            ////点击地点类型跳转到这个——————
            getRiskAnalyzeByPlaceType: function () {
                ...
                $("#block-tabs-sh").hide();
                $.ajax({
                    url: projectName + "/RiskControlPlaneByMonthControl/getRiskAnalyzeByPlaceType", //发送请求的地址。
                    type: "POST", //(默认: "GET") 请求方式 ("POST" 或 "GET")， 默认为 "GET"。注意：其它 HTTP 请求方法，如 PUT 和 DELETE 也可以使用，但仅部分浏览器支持。
                    data: {"frameWorkID": _self.riskSite.frameWorkID,},//发送到服务器的数据。将自动转换为请求字符串格式。GET 请求中将附加在 URL 后。
                    dataType: "json",//预期服务器返回的数据类型。
                    async: "false",//(默认: true) 默认设置下，所有请求均为异步请求。如果需要发送同步请求，请将此选项设置为 false。注意，同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行。
                    success: function (mydata) {
                        //请求成功后的回调函数
                        _self.detailedList = mydata;
                        var optionsPie = myChart.getOption();
                        var a = [];
                        var res = [];
                        _self.count = _self.detailedList[0].placeCount;
                        for (var i = 0; i < _self.detailedList.length; i++) {
                            a.push(_self.detailedList[i].riskDamageName);
                            res.push({
                                value: _self.detailedList[i].placeCount,
//                                name: _self.damageList[i].fData_Name
                            })

                           // if (i < _self.detailedList.length - 1) {
                            //    if (_self.detailedList[i].placeCount < _self.detailedList[i + 1].placeCount) {
                            //        _self.count = _self.detailedList[i + 1].placeCount;
                            //    }
                        ...
                        /* for (var i = 0; i < _self.detailedList.length; i++) {
                             ...
                             res.push({
                                 value: _self.detailedList[i].placeCount,
                                 name: _self.detailedList[i].riskDamageName
                             })
                         }*/
...
//                                name: _self.damageList[i].fData_Name
        ...
        Cleanup:function() {
            ...
            tableToExcel:function(tableId){
                ...
                if(tableId == "general-table"){
                    name="灾害类型风险点报表.xls";
                }else if(tableId == "general-table1"){
                    name="地点类型风险点报表.xls";
                }
